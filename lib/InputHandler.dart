...
class InputHandler {
  ...
  void _moveDir(Direction dir) {
    ...
    if (dir == Direction.up) {
      if (maze.isObstacle(chicken.mapPos.x, chicken.mapPos.y - 1)) {
        //Make a hole?
        Tile tile = maze.getTileFromLayer(0,
            chicken.mapPos.x, chicken.mapPos.y - 1);
    ...
    if (dir == Direction.left) {
      if (maze.isObstacle(chicken.mapPos.x - 1, chicken.mapPos.y)) {
        //Make a hole?
        Tile tile = maze.getTileFromLayer(0,
            chicken.mapPos.x - 1, chicken.mapPos.y);
  ...
  void touched(double xp, double yp) {
    var centerX = chicken.screenPos.x * raster * game.scaleFactor + raster * game.scaleFactor / 2;
    var centerY = chicken.screenPos.y * raster * game.scaleFactor + raster * game.scaleFactor / 2;
    var dX = xp - centerX;
    var dY = yp - centerY;
    if (dX.abs() > dY.abs()) { //Horiz
      if (dX > 0) { // R
